...
- [改进] A 股情报扩展新增 lazy `astock_data` provider adapter、结构化状态 schema 和内存/文件缓存 manager，区分 `empty`、`stale` 与 provider unavailable。
...
- [新功能] 回测汇总新增 v2 可交易性指标：基准覆盖率、平均超额收益、alpha 命中率、平均交易成本、不可成交率、可成交胜率（剔除封板不可成交后的胜率），由 per-result benchmark/excess/unfillable/cost 聚合，`backtest_summaries` 追加列迁移并经 `PerformanceMetrics`/归因 API 暴露、同步 Web 类型；v1 多为 NULL，旧汇总行重算后回填。
- [文档] 新增 [Fork 能力增强总览](fork-enhancements.md)，汇总本 fork 在上游基础上新增的能力（A 股情报、结构化决策信号 B、数据质量策略 C、回测评估 2.0 D、运行流/数据源健康观测）与配置开关、功能-PR 对照，并在 README 增加入口。
- [改进] 首页与个股/大盘复盘统一 A 股红涨绿跌配色，大盘复盘“复盘摘要”卡改为纯文本渲染，去除 Markdown 语法泄漏。
- [改进] 大盘复盘详情拆分为 报告摘要 / 结构化数据 / 原文报告 / 证据与诊断 标签页，消除同屏长滚动与结构化数据/原文重复。
- [改进] 首页移动端将次要操作（策略、推送通知、重新分析、追问 AI、历史趋势）收纳进“更多”溢出菜单，完整报告入口保留常驻。
- [修复] 大盘复盘模板兜底：结论改用复合盘面信号并在指数方向与涨跌家数广度背离时点明、指数振幅缺失时按 (最高-最低)/昨收 兜底、指数表成交额标注“成份”口径、页脚区分“数据日期/报告生成”、报告产出统一规范化字面换行与多余空行。
- [改进] 大盘复盘模板与 LLM prompt 严谨性：成交额改为基准感知（无基准时标注“绝对口径”）、板块新增“主线研判”综述、策略框架与风险提示随当日盘面信号动态生成，prompt 要求指数与广度背离时以广度定调并明确量能口径。
- [改进] 个股分析报告新增统一“报告可信度”结论条（合成数据质量等级与运行诊断状态），透明度明细卡保留为下钻，移除报告末尾孤立的模型标记行。
